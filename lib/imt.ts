import {IMTHashFunction, IMTMerkleProof, IMTNode} from './imt.types';
...
export default class IMT {
  ...
  constructor(
    ...
    if (leaves.length > arity ** depth) {
      throw new Error(
        `The tree cannot contain more than ${arity ** depth} leaves`,
      );
    }
    ...
    if (leaves.length > 0) {
      ...
      for (let level = 0; level < depth; level += 1) {
        for (
          let index = 0;
          index < Math.ceil(this._nodes[level].length / arity);
          index += 1
        ) {
          ...
          for (let i = 0; i < arity; i += 1) {
            children.push(
              this._nodes[level][position + i] ?? this.zeroes[level],
            );
          }
